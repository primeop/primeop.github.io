...
document.addEventListener('mousemove', (e) => {
    ...
    // Position hacker character relative to scroll
    const scrollPercent = (window.pageYOffset / (document.documentElement.scrollHeight - window.innerHeight)) * 100;
    hackerCharacter.style.left = (mouseX - 40) + 'px';
    hackerCharacter.style.top = (window.pageYOffset + mouseY - 40) + 'px';
});

// Show hacker character on scroll
let scrollTimeout;
window.addEventListener('scroll', () => {
    hackerCharacter.classList.add('visible');

    clearTimeout(scrollTimeout);
    scrollTimeout = setTimeout(() => {
        hackerCharacter.classList.remove('visible');
    }, 3000);
});
...
window.addEventListener('load', () => {
    ...
    // Apply slide animations to cards
    ...
    // Apply scale-in to accordion items
    document.querySelectorAll('.accordion-item').forEach((item) => {
        item.classList.add('scale-in');
        animationObserver.observe(item);
    });

    // Apply animations to skill items
...
if (contactForm) {
    contactForm.addEventListener('submit', async function(e) {
        ...
        if (!action || action.includes('REPLACE_FORM_ID')) {
            showNotification('Please configure your Formspree form ID in index.html', 'error');
            return;
        }
        ...
        try {
            ...
            if (res.ok) {
                ...
                const msg = data && data.errors ? data.errors.map(e => e.message).join(', ') : 'Submission failed. Try again later.';
